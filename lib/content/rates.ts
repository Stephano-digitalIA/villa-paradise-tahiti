...
export const RATES_CONTENT_GROUPS: ContentGroup[] = [
  ...
  },
  {
    ...
    fields: [
      ...
      { key: 'rates.grid.high.window', label: 'Haute saison, période (utilisé seulement si aucune saison n\'est définie dans Réglages)' },
      { key: 'rates.grid.high.blurb', label: 'Haute saison, texte', multiline: true },
      { key: 'rates.grid.peak.name', label: 'Très haute saison, nom' },
      { key: 'rates.grid.peak.window', label: 'Très haute saison, période (utilisé seulement si aucune saison n\'est définie dans Réglages)' },
